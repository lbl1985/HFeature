function master_train_kth(vid_file_string, path)
...
checkFolder(bases_path);    checkFolder(unsup_train_data_path);
...
for i = 1:network.num_isa_layers %1 and 2 layers (since 3rd layer is the same size)       
    ...
    extract_unsupervised_training_data_kth(vid_file_string, data_path_file_name{i}, params_ex);
...
send_mail_message('herbert19lee', 'HFfeature Train finished', datestr(now));